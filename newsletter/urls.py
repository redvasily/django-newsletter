...
urlpatterns = patterns('newsletter.views',
    ...
    url(r'^(?P<newsletter_slug>[-\w]+)/subscription/(?P<subscription_id>\d+)/(?P<action>[a-z]+)/activate/(?P<activation_code>[a-zA-Z0-9]+)/$', 'update_subscription', name='newsletter_update_activate'),
    url(r'^(?P<newsletter_slug>[-\w]+)/subscription/(?P<subscription_id>\d+)/(?P<action>[a-z]+)/activate/$', 'update_subscription', name='newsletter_update'),
